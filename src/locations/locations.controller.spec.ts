  ...
  describe('create', () => {
    ...
    it('should return success response when location is created', async () => {
      ...
      jest
        ...
        .mockResolvedValue({ id: 1, city: 'Pune', userId: 1 }); // Simulate created location

      const response = await controller.create(postData, {
        user: user,
      });
      expect(response).toEqual({
        success: true,
        data: { id: 1, city: 'Pune', userId: 1 },
      });
    });
  });
  ...
  describe('get', () => {
    it('should return list of city for user', async () => {
      jest.spyOn(locationService, 'findAllByUserId').mockResolvedValue([
        { id: 1, city: 'Pune', userId: 1 },
        { id: 2, city: 'Mumbai', userId: 1 },
      ]);
      const response = await controller.get({ user: user });
      expect(response).toEqual({
        success: true,
        data: [
          { id: 1, city: 'Pune', userId: 1 },
